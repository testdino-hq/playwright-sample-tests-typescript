...
Contains **49 `test.step()` calls** (so traces have a readable step breakdown) and **22 `testInfo.attach()` calls** across PNG, JSON, HTML, text and real files. The workflow also uploads raw `test-results/` as a separate artifact, so traces can be opened locally with `npx playwright show-trace`. Traces reach TestDino automatically — `tdpw test` uploads screenshots, videos and traces unless `--no-artifacts` is passed.
...
├── attachments-tests.yml     4 shards × 2 workers, also keeps raw test-results/
├── feature-split-tests.yml   7 jobs across 5 splits
└── test.yml                  the repo's own suite (manual only)
```
...
There is no blob reporter and no merge job. Each shard reports its own results, and
TestDino groups them into a single run using a shared run id.

```
Each shard
──────────
npx tdpw test --ci-run-id "gh-<run_id>-<run_attempt>" <spec> --shard=i/N
    │
    ├── results stream to TestDino as the shard runs
    ├── screenshots, videos and traces upload automatically
    │      (pass --no-artifacts to skip them)
    └── the HTML report is also kept as a GitHub artifact for 14 days

TestDino groups every shard sharing the same --ci-run-id into one run.
```

`run_attempt` is part of the id, so re-running a workflow creates a new TestDino run
rather than merging into the original.

The split mode suite groups on `--split-id` instead, because its jobs run different
commands rather than shards of one command.

The attachments suite additionally uploads its raw `test-results/` directory as a
GitHub artifact, so traces can be opened locally with `npx playwright show-trace`.
