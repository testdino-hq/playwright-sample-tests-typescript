...
var CACHE_FILE = (0, import_path4.join)((0, import_os3.tmpdir)(), "testdino-playwright-update-check.json");
function readCachedLatest() {
  try {
    const parsed = JSON.parse((0, import_fs.readFileSync)(CACHE_FILE, "utf-8"));
...
function writeCachedLatest(latest) {
  try {
    ...
    (0, import_fs.writeFileSync)(CACHE_FILE, JSON.stringify(entry), "utf-8");
  } catch {
  }
}
...
var TestdinoReporter = class _TestdinoReporter {
  ...
  constructor(config = {}) {
    ...
    this.runId = this.injectedRunId ?? (0, import_crypto2.randomUUID)();
  ...
  async onEnd(result) {
    ...
    if (resultPath) {
      try {
        (0, import_fs2.writeFileSync)(resultPath, JSON.stringify({ totalTests: this.executedTests }));
      } catch {
      }
    }
    if (!this.initPromise) {
